...
// ─────────────────────────────────────────────────────────────────────────────
final _posterActiveTasksProvider =
...
class PosterHomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final firstName =
        userAsync.valueOrNull?.displayName?.split(' ').first ?? 'Neighbour';
        ...
        slivers: [
          ...
          ...tasksAsync.when(
            ...
            data: (tasks) {
              ...
              final needsAction =
                  tasks.where((t) => t.isOpen && t.bidCount > 0).toList();
...
class _PosterTaskCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final category =
        AppCategories.all.where((c) => c.id == task.categoryId).firstOrNull;
