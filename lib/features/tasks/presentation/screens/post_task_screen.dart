...
class _PostTaskScreenState extends ConsumerState<PostTaskScreen> {
    ...
    try {
      ...
      if (user == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Please sign in again before posting a task.'),
      ...
      final task = TaskModel(
        ...
        assignedProviderName:
            _isDirectHire ? widget.directHireProviderName : null,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              _isDirectHire
                  ? 'Could not send the hire request. Please try again.'
                  : 'Could not post your task. Please try again.',
            ),
            backgroundColor: AppColors.error,
          ),
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_isDirectHire
            ? 'Hire Provider  (${_step + 1}/$_totalSteps)'
            : 'Post a Task  (${_step + 1}/$_totalSteps)'),
        leading: _step == 0
            ? IconButton(
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: [
                  ...
                  _StepLocation(
                      formKey: _formKeys[2],
                      onLocationChanged: (l) => _locationLabel = l,
                      onHoodChanged: (h) => _neighbourhood = h),
                  _StepBudget(
                      formKey: _formKeys[3],
                      budgetMin: _budgetMin,
            ...
            Padding(
              ...
              child: AppButton(
                label: _step < _totalSteps - 1
                    ? 'Next'
                    : (_isDirectHire ? 'Send Hire Request' : 'Post Task'),
                isLoading: _loading,
                onPressed: _next,
              ),
            ),
        ...
        child: Column(
          ...
          children: [
            ...
            FormField<String>(
              ...
              validator: (_) =>
                  _selected == null ? 'Please select a category' : null,
...
class _StepDescriptionState extends State<_StepDescription> {
  ...
  Future<void> _pick() async {
    final imgs = await ImagePicker().pickMultiImage(
        maxWidth: 1024, imageQuality: 80, limit: AppConstants.maxTaskPhotos);
    if (imgs.isEmpty) return;

    final files =
        imgs.map((x) => File(x.path)).take(AppConstants.maxTaskPhotos).toList();
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(
                hintText: 'e.g. Clean my 3-room HDB flat'),
  ...
  Widget build(BuildContext context) => Form(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextFormField(
            controller: _locCtrl,
            decoration: const InputDecoration(
                hintText: 'Blk 123 Ang Mo Kio Ave 6 #05-45',
                prefixIcon: Icon(Icons.location_on_outlined)),
            onChanged: widget.onLocationChanged,
            validator: (v) => (v == null || v.trim().isEmpty)
                ? 'Enter the task location'
                : null,
          ),
          const SizedBox(height: 20),
...
class _StepBudgetState extends State<_StepBudget> {
  ...
  Widget build(BuildContext context) => Form(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 8),
          Text(
              'Set a fair budget. Providers will see this and submit their quotes.',
            ...
            Expanded(
                child: Column(
                  ...
                  TextFormField(
                    ...
                    decoration: const InputDecoration(
                        prefixText: 'S\$ ', helperText: 'Min S\$5'),
                    onChanged: (v) {
                      _min = double.tryParse(v) ?? 0;
          ...
          CheckboxListTile(
            ...
            title: const Text('Set a budget range',
                style: TextStyle(fontSize: 14)),
            onChanged: (v) => setState(() {
              _range = v!;
              if (!_range) {
                _max = null;
                widget.onBudgetChanged(_min, null);
              }
            }),
          ),
        ]),
...
class _StepReview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _ReviewRow('Budget',
          budgetMax != null ? 'S\$$budgetMin–S\$$budgetMax' : 'S\$$budgetMin'),
